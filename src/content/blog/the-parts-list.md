...
**A 360-degree scanning lidar.** The four TF-Lunas watch four directions, which means they don't watch the arcs in between. The clean fix is a spinning lidar like a Slamtec RPLidar: one unit painting a full horizontal circle instead of four fixed beams. So I went down the rabbit hole. The cheap triangulation models get washed out by sunlight, so the one you actually want outdoors is the time-of-flight S1, which ArduPilot supports directly and which runs $649 at SeeedStudio. The price isn't even the dealbreaker. The mounting is. A scanning lidar needs a clear horizontal plane to sweep, and on a quad that plane is full of arms and spinning props. You end up putting it on a mast above the props or a pole below the landing gear, both of which pile height, weight, and CG grief onto an already-loaded airframe. So I'm taking baby steps. Get it flying, log everything, find where the four-beam coverage actually bites me, and confirm it's a real problem for the way I actually fly before I spend $649 and a hundred grams solving a hypothetical one. If I hit a wall, I'll know exactly which upgrade earns its weight. Requirements by evidence, not by anxiety.
